...
vs = VectorSlicer(outputCol='sliced', indices=features)
...
lr = LogisticRegression(maxIter=10, featuresCol = "sliced", weightCol="label")
model = lr.fit(train)
predictions = model.transform(test)
result = predictions.select("label","prediction").rdd.map(lambda x: (float(x[0]), float(x[1]))).cache()
metrics = MulticlassMetrics(result)
...
svm = LinearSVC(maxIter=10, featuresCol = "sliced", weightCol="label")
model = svm.fit(train)
predictions = model.transform(test)
result = predictions.select("label","prediction").rdd.map(lambda x: (float(x[0]), float(x[1]))).cache()
metrics = MulticlassMetrics(result)
